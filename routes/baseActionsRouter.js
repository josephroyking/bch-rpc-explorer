    ...
    .then(function (result) {
      ...
      promises.push(
        new Promise(function (resolve, reject) {
          coreApi
            .getBlockByHashWithTransactions(result.hash, limit, offset)
            .then(function (result) {
              res.locals.result.getblock = result.getblock;
              res.locals.result.transactions = result.transactions;
              res.locals.result.txInputsByTransaction =
                result.txInputsByTransaction;
...
router.get("/block/:blockHash", function (req, res, next) {
  ...
  promises.push(
    new Promise(function (resolve, reject) {
      coreApi
        .getBlockByHashWithTransactions(blockHash, limit, offset)
        .then(function (result) {
          res.locals.result.getblock = result.getblock;
          res.locals.result.transactions = result.transactions;
          res.locals.result.txInputsByTransaction =
...
router.get("/tx/:transactionId", function (req, res, next) {
  ...
  coreApi
    .getRawTransactionsWithInputs([txid])
    .then(function (rawTxResult) {
      ...
      if (tx.confirmations == null) {
        promises.push(
          new Promise(function (resolve, reject) {
            coreApi
              .getMempoolTxDetails(txid)
              .then(function (mempoolDetails) {
...
router.get("/address/:address", function (req, res, next) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        var prefix =
          global.activeBlockchain == "main" ? "bitcoincash:" : "bchtest:";
  ...
  coreApi
    .getAddress(address)
    .then(function (validateaddressResult) {
        ...
        promises.push(
          new Promise(function (resolve, reject) {
            addressApi
              ...
              .then(function (addressDetailsResult) {
                ...
                if (addressDetails) {
                  ...
                  if (addressDetails.txids) {
                    ...
                    coreApi
                      .getRawTransactionsWithInputs(txids)
                      .then(function (rawTxResult) {
                        ...
                        for (
                          var i = 0;
                          i < rawTxResult.transactions.length;
                          i++
                        ) {
                        ...
                        if (coinbaseTxs.length > 0) {
                          ...
                          blockHeightsPromises.push(
                            new Promise(function (resolve2, reject2) {
                              coreApi
                                .getBlocksByHash(coinbaseTxBlockHashes)
                                .then(function (blocksByHashResult) {
                                  for (var txid in blockHashesByTxid) {
                                    if (
                                      blockHashesByTxid.hasOwnProperty(txid)
                                    ) {
                        ...
                        Promise.all(blockHeightsPromises)
                          .then(function () {
                            var addrGainsByTx = {};
                            var addrLossesByTx = {};
                            ...
                            for (
                              ...
                              for (var j = 0; j < tx.vout.length; j++) {
                                if (
                                  ...
                                  tx.vout[j].scriptPubKey.addresses.includes(
                                    address
                                  )
                                ) {
                                  ...
                                  addrGainsByTx[tx.txid] = addrGainsByTx[
                                    tx.txid
                                  ].plus(new Decimal(tx.vout[j].value));
                                }
                              }
                              ...
                              for (var j = 0; j < tx.vin.length; j++) {
                                ...
                                if (txInput != null) {
                                  if (
                                    txInput &&
                                    txInput.scriptPubKey &&
                                    txInput.scriptPubKey.addresses &&
                                    txInput.scriptPubKey.addresses.includes(
                                      address
                                    )
                                  ) {
      ...
      Promise.all(promises.map(utils.reflectPromise))
        .then(function () {
          res.render("address");
        })
        .catch(function (err) {
